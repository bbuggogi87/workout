/**
 * 파일명: app.js
 * 역할: 식단 플래너 도메인 총괄 통제 및 영양소 입체 분석 오케스트레이터 컨트롤러
 * 변경사항: 파이어베이스 종속성 소거, 60초 주기 로컬 자동 저장 엔진 탑재, 훈련 스케줄 영양소 공제 로직 완벽 보완
 */
...
/**
 * 시스템 초기 부팅 및 화면 디스플레이 마운트 총괄 함수
 */
export function finishInit() { 
    ...
    // [신규 핵심 기능] 60초 주기 기기 내부 스토리지 백그라운드 강제 자동 저장 타이머 가동
    setInterval(() => {
        saveToLocal();
        console.log("[Auto-Save] 60초 주기 로컬 세이프가드 엔진이 데이터를 안전하게 보호했습니다.");
    }, 60000);
}

/**
 * 가변형 식단 페이즈(Phase) 탭 인터페이스 렌더링 함수
 */
...
/**
 * 모바일 입력 왜곡 방지 및 터치 최적화 식품 중량 10g 증감 조절 함수
 */
...
/**
 * 특정 식단 페이즈 데이터를 타임라인 뷰 레이어에 바인딩 및 렌더링하는 함수
 */
export function loadPhase(phaseId) { 
    ...
    const cp = state.phases.find(p => p.id === phaseId); 
    if(!cp) return;
    ...
    cp.meals.forEach((meal, mIdx) => {
        ...
        // 훈련 일정 체크박스의 활성화 상태 여부 확인 메커니즘 적용
    ...
    // SortableJS 반응형 드래그 인터페이스 레이아웃 보존 규칙 연동
...
/**
 * 특정 일정을 훈련 스케줄로 지정하여 영양소 합산 가중치 공제 여부를 토글하는 함수
 */
...
/**
 * 현재 활성화된 식단 전체 식품 매크로 비율 실시간 정밀 연산 함수
 * 고도화 사양: 훈련 스케줄(isWorkout) 처리된 식사 카드는 루프 계산 대상에서 자동으로 배제 공제함
 */
export function calculateMacros() {
    ...
    if(cp) { 
        cp.meals.forEach(m => { 
            // [상위 호환 보완 로직] 해당 카드가 훈련 일정(isWorkout)일 경우 영양소 총합산 연산에서 자동 제외 처리
    ...
    if (!state.pieChartInstance && !document.getElementById('tab-analysis').classList.contains('hidden')) { 
        ...
        state.pieChartInstance.data.datasets[0].data = [tC, tP, tF]; 
...
export function initCalcDropdowns() {
    const cDrop = document.getElementById('calc-carb-src'); 
    const pDrop = document.getElementById('calc-pro-src'); 
    const fDrop = document.getElementById('calc-fat-src');
...
export function renderCustomSupps() {
    const container = document.getElementById('custom-supp-list'); 
    container.innerHTML = '';
...
// 브라우저 전역 네임스페이스 함수 윈도우 바인딩 규칙 완벽 유지
...
// 외부 오프라인 환경에 구애받지 않는 안전한 로컬 단독 부팅 실행
