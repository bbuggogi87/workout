/**
 * 파일명: app.js
 * 역할: 식단 플래너 총괄 통제, 체중 기록지 대시보드 및 엑셀 입출력 통합 컨트롤러
 * 변경사항: 중복 선언 구문 오류(SyntaxError) 전면 청소 및 두 도메인 간 유기적 바인딩 통합 완료
 */
...
// ==========================================
// 1. 브라우저 전역 윈도우 (window) 바인딩 (중복 제거 통합본)
// ==========================================
...
export function finishInit() { 
    ...
    if(state.userInfo.targetDate) { 
        ...
        if(dBadge) {
            ...
            dBadge.innerText = diff >= 0 ? `D-${diff}` : `D+${Math.abs(diff)}`;
        }
    }
...
export function switchMainTab(tabId) { 
    ...
    // 대상 탭 표시
    const targetEl = document.getElementById(tabId) || document.getElementById('pane-' + tabId);
    if(targetEl) {
        targetEl.classList.remove('hidden'); targetEl.classList.add('block');
    }
    ...
    tabs.forEach(t => { 
        ...
        if(btn) {
            btn.className = (t === tabId || t === tabId.replace('pane-', '')) 
                ? "px-5 py-3 rounded-xl text-base font-bold active-tab shrink-0 transition-colors" 
                : "px-5 py-3 rounded-xl text-base font-bold border border-slate-800 text-slate-400 hover:text-white shrink-0 transition-colors"; 
        }
    });
    
    if(tabId.includes('analysis')) calculateMacros();
    if(tabId.includes('weight')) {
...
// ==========================================
// 3. 기존 식단 플래너 핵심 비즈니스 로직 (원형 보존)
// ==========================================
export function renderPhaseTabs() { 
    const container = document.getElementById('phase-tabs-container'); if(!container) return; 
    container.innerHTML = ''; 
...
export function loadPhase(phaseId) { 
    ...
    if(descEl) descEl.innerText = cp.desc; 
    ...
    cp.meals.forEach((meal, mIdx) => { 
        ...
        <div class="flex items-stretch mb-6"> 
            <div class="relative flex flex-col items-center mr-4 sm:mr-6 w-10 shrink-0"> 
                <div class="w-px h-full bg-slate-800 absolute top-8 bottom-[-32px] z-0"></div> 
    ...
    if (typeof Sortable !== 'undefined' && document.getElementById('timeline-container')) { 
        ...
        window.timelineSortable = new Sortable(document.getElementById('timeline-container'), { 
            handle: '.drag-handle', animation: 250, ghostClass: 'opacity-10', delay: 150, delayOnTouchOnly: true, forceFallback: false, 
            onEnd: function (evt) { 
...
export function initCalcDropdowns() { 
    ...
    if(!cDrop) return; cDrop.innerHTML = ''; pDrop.innerHTML = ''; fDrop.innerHTML = ''; 
...
export function runSmartCalc(type) { 
    if(!document.getElementById(`calc-${type}-src`)) return; 
